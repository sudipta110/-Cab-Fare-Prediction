...
setwd("D:/edwisor/Cab Fare Prediction Project")
...
univ_dist(train$fare_amount)

# distrubution of  independent variable 'pickup_longitude'
univ_dist(train$pickup_longitude)

# distrubution of  independent variable 'pickup_latitude'
univ_dist(train$pickup_latitude)

# distrubution of  independent variable 'dropoff_longitude'
univ_dist(train$dropoff_longitude)

# distrubution of  independent variable 'dropoff_latitude'
univ_dist(train$dropoff_latitude)


#**********************Bivarite Analysis*************************#
#Jointplots for Bivariate Analysis.
WVPlots::ScatterHist(train, "fare_amount", "pickup_longitude", smoothmethod="lm", 
                     title="Bivariate Analysis")

WVPlots::ScatterHist(train, "fare_amount", "pickup_latitude", smoothmethod="lm", 
                     title="Bivariate Analysis")

WVPlots::ScatterHist(train, "fare_amount", "dropoff_longitude", smoothmethod="lm", 
                     title="Bivariate Analysis")

WVPlots::ScatterHist(train, "fare_amount", "dropoff_latitude", smoothmethod="lm", 
                     title="Bivariate Analysis")


#Some Violinplots to see spread of variables
ggplot(train, aes(fare_amount,pickup_longitude)) + geom_violin()

ggplot(train, aes(fare_amount,pickup_latitude)) + geom_violin()

ggplot(train, aes(fare_amount,dropoff_longitude)) + geom_violin()

ggplot(train, aes(fare_amount,dropoff_latitude)) + geom_violin()


#*****************Multivariate Analysis*********************#

ggpairs(train[,c('fare_amount','pickup_longitude','pickup_latitude','dropoff_longitude','dropoff_latitude')],title = 'Multivariate Analysis')
...
# #check multicollearity
 ...
 rmExcept(c("test","train","df",'df1','df2','test_data','train_data','test_pickup_datetime'))
 ###################Model Selection################
 ...
 #    mae       mse      rmse      mape 
 #  1.9092645 6.4185957 2.5334948 0.2340308
